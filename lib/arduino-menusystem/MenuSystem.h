 ...
 * 
 * Modified for GLUEHWOSTAT project
 */
...
public:
  ...
  virtual void render_numeric_display_menu_item(NumericDisplayMenuItem<float> const &menu_item) const = 0;
...
{
public:
  using FormatValueFnPtr = const String (*)(const T value);
...
public:
  ...
  //! @param name The name of the menu item.
  //! @param value Default value.
  NumericDisplayMenuItem(const char *basename, SelectFnPtr select_fn, FormatValueFnPtr format_value_fn,
                         T value)
